...
import com.mitbook.zkclient.connection.ZKConnection;
import com.mitbook.zkclient.connection.ZKConnectionImpl;
import com.mitbook.zkclient.event.ZKEventLock;
...
import com.mitbook.zkclient.exception.ZKNodeExistsException;
import com.mitbook.zkclient.listener.ZKChildCountListener;
import com.mitbook.zkclient.listener.ZKChildDataListener;
import com.mitbook.zkclient.listener.ZKListener;
...
import com.mitbook.zkclient.watcher.ZKWatcher;
...
public class ZKClient {
     ...
     *
     * @param path
     * @param watch true:将监听该节点的状态,包括创建,删除,修改节点数据,状态改变则会触发监听事件.
     *              false:不启用监听
     ...
     *
     * @param path  父节点路径
     * @param watch true:监听该节点子节点状态,包括删除此节点,创建子节点,删除子节点,状态改变则会触发监听事件.
     *              false:不启用监听
     ...
     * @param path       路径
     * @param data       数据
     * @param sequential true:会创建{@link CreateMode#EPHEMERAL_SEQUENTIAL}类型的节点.
     ...
     *
     * @param path                      路径
     * @param returnNullIfPathNotExists 值为true,在路径不存在的时候返回null,值为false,路径不存在的时候抛出异常
     * @return T
     */
    @SuppressWarnings("unchecked")
     ...
     */
    public Stat setData(final String path, Object data, final int version) {
        final byte[] datas = serializer.serialize(data);
        LOG.debug("set data path[" + path + "]");
        return (Stat) retryUntilConnected(new Callable<Object>() {

            @Override
            public Object call() throws Exception {
                Stat stat = getZooKeeper().setData(path, datas, version);
